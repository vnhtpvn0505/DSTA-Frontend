...
const DEFAULT_OPTIONS: { optionText: string; isCorrect: boolean }[] = [
...
export default function CreateQuestionDialog({
  ...
  const createMutation = useMutation({
    ...
    onError: (err: { response?: { data?: unknown }; message?: string }) => {
      ...
      let msg = 'Không thể tạo câu hỏi. Vui lòng thử lại.'
  ...
  useEffect(() => {
    if (open && categories.length > 0 && !form.getValues('categoryId')) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Tạo câu hỏi mới (UC22)</DialogTitle>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <select
                      ...
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-[#00284D] focus:outline-none focus:ring-1 focus:ring-[#00284D] disabled:bg-gray-50 disabled:opacity-70"
                    >
                      <option value="">
                        {categoriesLoading
                          ? 'Đang tải...'
                          : categoriesError
                            ? 'Lỗi tải danh mục'
                            : categories.length === 0
                              ? 'Chưa có miền năng lực (tạo trong tab Cài đặt)'
                              : 'Chọn miền năng lực'}
                      </option>
                      {categories.map((c) => (
            ...
            <DialogFooter className="gap-2 pt-4">
              ...
              >
                {createMutation.isPending ? 'Đang lưu...' : 'Lưu (Pending)'}
              </Button>
            </DialogFooter>
          </form>
        </Form>
