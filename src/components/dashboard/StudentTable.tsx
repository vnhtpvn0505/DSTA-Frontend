...
import { Eye, Filter, Search, X, FileBarChart } from 'lucide-react';
import type { StudentTableRow, StudentResultStatus } from '@/types/student';

const resultConfig: Record<
  StudentResultStatus,
  { label: string; className: string }
> = {
  PASS: { label: 'ĐẠT', className: 'bg-green-500 text-white' },
  PENDING: { label: 'ĐANG CHỜ', className: 'bg-amber-400 text-amber-900' },
  FAIL: { label: 'KHÔNG ĐẠT', className: 'bg-red-500 text-white' },
};

interface StudentTableProps {
  data: StudentTableRow[];
  onView?: (row: StudentTableRow) => void;
}

const DEFAULT_FILTERS = ['Điểm số', 'Bậc năng lực'];

export default function StudentTable({ data, onView }: StudentTableProps) {
      ...
      <div className="border-b border-gray-200 px-6 py-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="relative w-full sm:w-72">
            ...
            <input
              type="text"
              placeholder="Tìm kiếm sinh viên, mã số sinh viên, bậc năng lực"
              value={search}
              ...
              return (
                ...
                >
                  ...
                  <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-700">
                    <div>
                      <p>{row.submittedAt}</p>
                      <p className="text-xs text-gray-500">{row.submittedDate}</p>
                    </div>
                  </td>
                  ...
                  <td className="whitespace-nowrap px-6 py-4">
                    <span className="inline-flex items-center gap-1.5 rounded-lg bg-blue-50 px-2.5 py-1 text-xs font-medium text-[#00284D]">
